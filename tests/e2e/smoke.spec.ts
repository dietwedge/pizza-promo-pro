...
test('launches the desktop application and renders its first window', async () => {
  ...
  try {
    ...
    await expect(window.getByLabel('Content production workflow')).toBeVisible()
    await expect(window.getByLabel('Content direction')).toBeVisible()
    await expect(window.getByText('Review and deliver')).toBeVisible()
    const strategy='Post Idea 1: Friday family dinner\nTheme: Drive dinner orders\nVisual Suggestion: Show the real pizza'
    await window.getByLabel('Content direction').fill(strategy)
    await window.getByRole('button',{name:'Build 3 platform drafts'}).click()
    await expect(window.getByText(/Content Producer created 3 grounded drafts/)).toBeVisible()
    const strategyPackage=window.locator('.production-card').filter({hasText:'3 platform drafts'})
    await expect(strategyPackage).toBeVisible()
    expect(await strategyPackage.locator('.production-card-copy > p').evaluate(element=>getComputedStyle(element,'::before').content)).toContain('Internal direction')
    const publishableVariants=strategyPackage.locator('.variant-grid')
    await expect(publishableVariants).not.toContainText('Post Idea 1')
    await expect(publishableVariants).not.toContainText('Theme:')
    await expect(publishableVariants).not.toContainText('Visual Suggestion:')
    ...
    await reviewPackage.getByRole('button',{name:'Reopen for editing'}).click()
    await expect(window.getByText(/Visible media proof is unapproved and editable again/)).toBeVisible()
    await expect(reviewPackage.getByRole('button',{name:'Edit platform copy'})).toBeVisible()
    await reviewPackage.getByRole('button',{name:'Send for final review'}).click()
    await reviewPackage.getByRole('button',{name:'Approve package'}).click()
