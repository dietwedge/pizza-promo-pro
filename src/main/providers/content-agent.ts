...
export type CreativeBrief = { audience:string; message:string; tone:string; callToAction:string; visualDirection:string }
export type ContentAgentResult = { provider: string; concept: string; creativeBrief:CreativeBrief; variants: readonly { platform: AgentPlatform; copy: string }[]; mediaPrompts: readonly { kind: 'image' | 'video'; prompt: string }[]; suggestedTiming: { rationale: string; localHour: number }; sources: readonly FactSource[]; requiresHumanApproval: true }
