  ...
  async complete(messages:readonly AiMessage[]):Promise<AiCompletion>{
    ...
    try{
      ...
      if(this.config.provider==='openai'){
        const response=await fetch(`${this.config.endpoint.replace(/\/$/,'')}/responses`,{method:'POST',headers:{Authorization:`Bearer ${this.config.apiKey}`,'Content-Type':'application/json'},body:JSON.stringify({model:this.config.model,input:messages.map(({role,content})=>({role,content})),max_output_tokens:3200}),signal:controller.signal,redirect:'error'})
      ...
      const response=await fetch(`${this.config.endpoint.replace(/\/$/,'')}/chat/completions`,{method:'POST',headers:{Authorization:`Bearer ${this.config.apiKey}`,'Content-Type':'application/json'},body:JSON.stringify({model:this.config.model,messages,temperature:.5,max_tokens:3200}),signal:controller.signal,redirect:'error'})
