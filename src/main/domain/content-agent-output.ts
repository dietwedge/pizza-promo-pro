...
export function parseContentAgentOutput(text:string,requested:readonly AgentPlatform[],provider:string,model:string,sources:readonly FactSource[]):ContentAgentResult{
  ...
  return {provider:`${provider}:${model}`,concept:parsed.concept,variants,mediaPrompts:parsed.mediaPrompts,suggestedTiming:parsed.suggestedTiming,sources,requiresHumanApproval:true}
}
