...
import { estimateHiggsfieldCredits, generateWithHiggsfield, getHiggsfieldStatus, higgsfieldProfile } from './higgsfield-cli-service'

type Kind='image'|'video'
type Aspect='1:1'|'4:5'|'9:16'|'16:9'
const MAX_DOWNLOAD_BYTES=150*1024*1024

function profileLabel(kind:Kind):string{return kind==='image'?'GPT Image 2':'Seedance 2.0'}

export async function estimateHiggsfieldMedia(input:{prompt:string;kind:Kind;aspectRatio:Aspect}):Promise<Record<string,unknown>>{
  const status=await getHiggsfieldStatus()
  if(status.state!=='ready')throw new Error(status.message)
  const profile=higgsfieldProfile(input.kind,input.aspectRatio),credits=await estimateHiggsfieldCredits(input.prompt,profile)
  return {provider:'higgsfield',kind:input.kind,model:profile.model,modelLabel:profileLabel(input.kind),aspectRatio:input.aspectRatio,credits,settings:profile.settings}
}
...
export async function generateHiggsfieldMedia(input:{contentItemId:string;prompt:string;kind:Kind;aspectRatio:Aspect;maxCredits:number;confirmSpend:true;confirmReview:true}):Promise<Record<string,unknown>>{
  ...
  database.prepare('INSERT INTO generation_jobs (id, content_item_id, provider, model, prompt, source_asset_ids_json, settings_json, status, started_at, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(jobId,input.contentItemId,'higgsfield',profile.model,input.prompt,'[]',JSON.stringify({...profile.settings,kind:input.kind,aspectRatio:input.aspectRatio,confirmedCredits:input.maxCredits}),'running',now,now,now)
  database.prepare('UPDATE content_items SET status = ?, updated_at = ? WHERE id = ?').run('media_generation',now,input.contentItemId)
  try{
    const generated=await generateWithHiggsfield(input.prompt,profile),download=await downloadMedia(generated.remoteUrl,input.kind),mediaId=randomUUID(),root=join(app.getPath('userData'),'media','generated'),path=join(root,`${mediaId}${download.extension}`),completed=Date.now()
    mkdirSync(root,{recursive:true});writeFileSync(path,download.bytes,{mode:0o600})
    database.exec('BEGIN IMMEDIATE')
    try{
      database.prepare('INSERT INTO media_assets (id, business_id, kind, local_path, original_filename, mime_type, byte_size, checksum_sha256, source, metadata_json, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(mediaId,content.business_id,input.kind,path,`${content.title}-${profile.model}${download.extension}`,download.mimeType,download.bytes.length,download.checksum,'higgsfield_generation',JSON.stringify({provider:'higgsfield',model:profile.model,prompt:input.prompt,aspectRatio:input.aspectRatio,credits}),completed,completed)
      database.prepare('INSERT INTO generation_outputs (id, generation_job_id, media_asset_id, provider_output_id, metadata_json, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)').run(randomUUID(),jobId,mediaId,generated.providerOutputId??null,JSON.stringify({model:profile.model,kind:input.kind,aspectRatio:input.aspectRatio,credits}),completed,completed)
    ...
    audit('media.higgsfield.generated','content_items',input.contentItemId,{jobId,mediaId,model:profile.model,kind:input.kind,credits,requiresReview:true})
    return {jobId,mediaId,status:'completed',provider:'higgsfield',model:profile.model,kind:input.kind,credits,requiresReview:true}
  }catch(error){const failed=Date.now(),message=error instanceof Error?error.message:'Higgsfield generation failed.';database.prepare('UPDATE generation_jobs SET status = ?, error_message = ?, completed_at = ?, updated_at = ? WHERE id = ?').run('failed',message,failed,failed,jobId);database.prepare('UPDATE content_items SET status = ?, updated_at = ? WHERE id = ?').run('draft',failed,input.contentItemId);audit('media.higgsfield.failed','content_items',input.contentItemId,{jobId,model:profile.model,kind:input.kind});throw error}
}
