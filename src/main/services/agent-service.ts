...
function factualContext(): FactSource[] {
  ...
  if(menu) sources.push({type:'menu_item',id:menu.id,label:menu.name,facts:{name:menu.name,description:menu.description,priceCents:menu.price_cents,currency:menu.currency}})
...
export async function produceContentPackage(objective:string,platforms:AgentPlatform[]):Promise<Record<string,unknown>> {
  const database=getDatabase(), sources=factualContext(), agent=new MockContentAgent()
