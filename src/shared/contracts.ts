...
export const ipcContracts = {
  ...
  'menu:previewUrl': {request:z.object({url:z.url({protocol:/^https?$/}).max(2048)}).strict(),response:z.object({sourceUrl:z.url({protocol:/^https?$/}),items:z.array(z.object({name:z.string(),description:z.string(),priceCents:z.number().int().nonnegative().nullable(),currency:z.string(),selected:z.boolean()})),message:z.string()})},
