...
export const ipcContracts = {
  ...
  'media:list': {request:z.object({}).strict(),response:z.array(recordSchema)},
  'menu:previewUrl': {request:z.object({url:z.url({protocol:/^https?$/}).max(2048)}).strict(),response:z.object({sourceUrl:z.url({protocol:/^https?$/}),provider:z.enum(['clover','square','slice','toast','other']),providerLabel:z.string(),items:z.array(z.object({name:z.string(),description:z.string(),priceCents:z.number().int().nonnegative().nullable(),currency:z.string(),selected:z.boolean()})),message:z.string()})},
  'menu:importPreview': {request:z.object({items:z.array(z.object({name:z.string().trim().min(1).max(200),description:z.string().max(2000),priceCents:z.number().int().nonnegative().max(10_000_000),currency:z.string().trim().length(3)})).min(1).max(200)}).strict(),response:z.object({imported:z.number().int().positive()})},
  ...
  'content:updateDraft': {request:z.object({contentItemId:idSchema,title:z.string().trim().min(3).max(140),brief:z.string().trim().min(10).max(2000),regenerateVariants:z.literal(true)}).strict(),response:recordSchema},
  'content:delete': {request:z.object({contentItemId:idSchema,confirmDelete:z.literal(true)}).strict(),response:z.object({deleted:z.literal(true)})},
